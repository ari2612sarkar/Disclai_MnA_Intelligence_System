...
export class DashboardView {
  ...
  private renderDashboard(txn: any, comp: any) {
          ...
          <div class="card-body">
            ${hasVerdict && verdict.company_a_recommendations.length > 0 ? `
              <div style="display: flex; flex-direction: column; gap: 10px;">
                ${verdict.company_a_recommendations.slice(0, 5).map((rec: any) => `
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${hasVerdict && verdict.company_b_recommendations.length > 0 ? `
              <div style="display: flex; flex-direction: column; gap: 10px;">
                ${verdict.company_b_recommendations.slice(0, 5).map((rec: any) => `
  ...
  private renderFindingsTable(findings: any[]) {
    return `
  ...
  bindEvents() {
    ...
    this.container.querySelector('#view-full-analysis')?.addEventListener('click', (e) => {
      ...
      if (compId) this.app.goToComparisonDetail(compId);
    });
  }
}