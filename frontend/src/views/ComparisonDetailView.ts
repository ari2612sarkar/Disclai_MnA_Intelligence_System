import { api } from '@/api';
import { formatCurrency, formatPercentage, formatDuration, formatDate, getSeverityClass, truncate } from '@/utils/helpers';

export class ComparisonDetailView {
  private container: HTMLElement;
  private app: any;
  private comparisonId: string;
  private comparison: any = null;
  private activeTab = 'findings';

  constructor(container: HTMLElement, app: any, comparisonId: string) {
  ...
  private renderVerdictHeader(verdict: any) {
    return `
  ...
  private renderRisksTab() {
    ...
    return cross_document_risks.map((r: any) => `
  ...
  private renderDisclosureTab() {
    ...
    return disclosure_issues.map((i: any) => `
          ...
          <div class="card">
            ...
            <div class="card-body">
              ${verdict.company_a_advantages.length > 0 ? `
                <ul style="margin-left: 16px;">${verdict.company_a_advantages.map((a: string) => `<li style="margin-bottom: 8px;">${a}</li>`).join('')}</ul>
          ...
          <div class="card">
            ...
            <div class="card-body">
              ${verdict.company_a_weaknesses.length > 0 ? `
                <ul style="margin-left: 16px;">${verdict.company_a_weaknesses.map((w: string) => `<li style="margin-bottom: 8px; color: var(--color-critical);">${w}</li>`).join('')}</ul>
          ...
          <div class="card">
            ...
            <div class="card-body">
              ${verdict.company_b_advantages.length > 0 ? `
                <ul style="margin-left: 16px;">${verdict.company_b_advantages.map((a: string) => `<li style="margin-bottom: 8px;">${a}</li>`).join('')}</ul>
              ` : '<p class="text-muted">No advantages identified</p>'}
            </div>
          </div>
          <div class="card">
            ...
            <div class="card-body">
              ${verdict.company_b_weaknesses.length > 0 ? `
                <ul style="margin-left: 16px;">${verdict.company_b_weaknesses.map((w: string) => `<li style="margin-bottom: 8px; color: var(--color-critical);">${w}</li>`).join('')}</ul>
              ` : '<p class="text-muted">No weaknesses identified</p>'}
            </div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${verdict.recommended_actions.length > 0 ? `
              <ol style="margin-left: 16px;">${verdict.recommended_actions.map((a: string) => `<li style="margin-bottom: 8px;">${a}</li>`).join('')}</ol>
  ...
  private formatPosition(pos: any): string {
    if (!pos) return '—';
    if (pos.percentage !== undefined) return `${pos.percentage}%`;
    if (pos.amount !== undefined) return formatCurrency(pos.amount);
    if (pos.duration) return pos.duration;
    return '—';
  }
