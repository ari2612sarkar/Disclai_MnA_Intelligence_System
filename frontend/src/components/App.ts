import { api } from '@/api';
import { formatCurrency, formatPercentage, formatDuration, formatDate, getSeverityClass, getStatusClass, truncate } from '@/utils/helpers';
...
type View = 'dashboard' | 'transactions' | 'comparisons' | 'documents' | 'comparison-detail';
...
export class App {
  ...
  private currentComparisonId: string | null = null;
  ...
  private toggleSidebar() {
    ...
    const sidebar = this.root.querySelector('#sidebar') as HTMLElement;
  ...
  private async renderView(view: View) {
    const content = this.root.querySelector('#content') as HTMLElement;
    const headerActions = this.root.querySelector('#header-actions') as HTMLElement;
  ...
  setCurrentTransactionId(id: string) { this.currentTransactionId = id; }
  getCurrentComparisonId() { return this.currentComparisonId; }
  setCurrentComparisonId(id: string) { this.currentComparisonId = id; }

  // Navigation helpers
  goToComparisonDetail(comparisonId: string) {
    this.currentComparisonId = comparisonId;
    this.switchView('comparison-detail');
  }

  goToDocuments(transactionId: string) {
