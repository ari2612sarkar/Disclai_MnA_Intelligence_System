...
export class NewComparisonModal {
  ...
  private getModalHtml(): string {
    const optionsHtml = this.transactions.map(t =>
      `<option value="${t.id}">${t.name} (${t.company_a_name} vs ${t.company_b_name})</option>`
    ).join('');
  ...
  private render() {
    const modalsContainer = this.app.root.querySelector('#modals') as HTMLElement;
  ...
  private bindEvents() {
    ...
    const txnSelect = this.overlay?.querySelector('#txn-select') as HTMLSelectElement;
    const runASelect = this.overlay?.querySelector('#run-a-select') as HTMLSelectElement;
    const runBSelect = this.overlay?.querySelector('#run-b-select') as HTMLSelectElement;
    ...
    txnSelect?.addEventListener('change', async () => {
      ...
      try {
        ...
        runASelect.innerHTML = '<option value="">Select Company A run</option>' +
          runsA.map((r: any) => `<option value="${r.id}">${r.id.slice(0,8)} - ${r.provisions_extracted} positions - ${new Date(r.started_at).toLocaleDateString()}</option>`).join('');
        runBSelect.innerHTML = '<option value="">Select Company B run</option>' +
          runsB.map((r: any) => `<option value="${r.id}">${r.id.slice(0,8)} - ${r.provisions_extracted} positions - ${new Date(r.started_at).toLocaleDateString()}</option>`).join('');
  ...
  private updateSubmitBtn(submitBtn: HTMLButtonElement, runASelect: HTMLSelectElement, runBSelect: HTMLSelectElement) {
    submitBtn.disabled = !runASelect.value || !runBSelect.value;
  }

  private async getCompanyRuns(companyId: string) {
