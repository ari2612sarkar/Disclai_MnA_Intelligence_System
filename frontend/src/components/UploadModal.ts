...
export class UploadModal {
  ...
  constructor(app: any, options?: { transactionId?: string; companyId?: string; role?: string }) {
    this.app = app;
  ...
  private bindEvents() {
    ...
    const fileInput = this.overlay?.querySelector('#file-input') as HTMLInputElement;
    const uploadZone = this.overlay?.querySelector('#upload-zone') as HTMLElement;
    const fileNameEl = this.overlay?.querySelector('#file-name') as HTMLElement;
    ...
    formEl?.addEventListener('submit', async (e) => {
      ...
      formData.append('role_in_document', (this.overlay?.querySelector('#role-select') as HTMLSelectElement).value);
      formData.append('document_type', (this.overlay?.querySelector('#doc-type-select') as HTMLSelectElement).value);

      try {
        const result = await api.documents.upload(fileInput.files[0], {
          transaction_id: this.transactionId,
          company_id: this.companyId,
          role_in_document: (this.overlay?.querySelector('#role-select') as HTMLSelectElement).value,
          document_type: (this.overlay?.querySelector('#doc-type-select') as HTMLSelectElement).value,
        });
  ...
  private updateFileName(fileInput: HTMLInputElement, fileNameEl: HTMLElement, submitBtn: HTMLButtonElement) {
